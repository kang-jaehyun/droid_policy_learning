...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if subgoal_config is not None:
            self.subgoal_config = subgoal_config
...
class DROIDDataset(SequenceDataset):
    ...
    def get_item(self, index):
        ...
        if self.goal_mode == "last":
            ...
            if num_options <= self.seq_length:
                ...
                if goal_index >= end_index_in_demo:
                    ...
                    # If it happens that you sample a goal_index within seq_length away from the current index, compute appropriate action padding
                    action_padding_len = self.seq_length - geom_sample_index - 1
        elif self.subgoal_config.enabled:
        ...
        if self.skill_config.enabled:
            task_name = self.hdf5_path.split('/')[-3]
            demo_id = self.hdf5_path.split('/')[-2]
            meta['goal'] = {}
        ...
        if self.subgoal_config.enabled:
            ...
            meta["goal"] = {k: goal[k][0] for k in goal}  # remove sequence dimension for goal
